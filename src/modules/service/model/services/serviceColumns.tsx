...
interface Props {
  handleUpdateOpen(data: IService): void
  mutate: KeyedMutator<any>
  trigger(id: number): Promise<any>
}

export function serviceColumns({handleUpdateOpen, trigger, mutate}: Props): GridColDef<IService>[] {
  return [
    ...
    {field: 'price', headerName: 'Цена', renderCell: ({row: {price}}) => <>{price} смн.</>, flex: 1},
    {field: 'unit', headerName: 'Ед. измер.', flex: 1},
    {field: 'sku', headerName: 'Код услуги', flex: 1},
    {
      field: 'actions',
      type: 'actions',
      width: 150,
      getActions: ({row}) => [
        <Link href={`/main/services/${row.slug}`}>
          <GridActionsCellItem
            title="Изображении"
            label="Изображении"
